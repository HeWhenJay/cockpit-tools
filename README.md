...
本 fork 增加了一个独立的 `dsh-plugin/`：把 API Service 的本地多渠道路由能力适配到 DeepSeek Harness。它默认只监听 `127.0.0.1`，可显式切换到 `0.0.0.0` 供可信局域网共享；API Key 只从本机环境变量读取，不写入配置示例、日志或仓库。普通低价中转站可配置为优先渠道，正常价格的官方 API 或其他账号可标记为 `backup` 保底。模型不限 GPT：插件提供 OpenAI-compatible 本地网关，DSH 内置 `llm-pi-ai` 指向该网关后，可以共同服务 GPT、Claude、Gemini、DeepSeek、Qwen、Kimi、Grok 等模型。详见 [`dsh-plugin/README.md`](dsh-plugin/README.md) 和 [`docs/DSH_PLUGIN_SCOPE.md`](docs/DSH_PLUGIN_SCOPE.md)。
